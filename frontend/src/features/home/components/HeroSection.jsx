export function HeroSection({ hero, onBookCall }) {
  return (
    <section id="home" className="min-h-screen flex items-center pt-[132px] px-[clamp(20px,6vw,96px)] pb-[72px] scroll-mt-[90px]">
      <div className="flex gap-[clamp(28px,5vw,72px)] items-center w-full max-w-[1280px] mx-auto flex-wrap">
        <div className="flex-[1_1_480px] min-w-[300px]">
          ...
          <h1 data-reveal className="font-display font-bold text-[clamp(40px,6.6vw,84px)] leading-[1.02] tracking-[-.035em] mt-[22px] text-[#f4f9f6]">
            {hero.headline.top}{' '}
            <span className="gradient-text">{hero.headline.accent}</span>{' '}
            <span className="text-[#f4f9f6]">{hero.headline.rest}</span>
          </h1>

          <p data-reveal className="max-w-[560px] mt-[26px] text-[clamp(16px,2vw,19px)] leading-[1.7] text-[#9fb0a7]">
            {hero.description}
          </p>
          ...
          <div data-reveal className="flex gap-[14px] flex-wrap mt-[34px]">
            <button
              type="button"
              onClick={onBookCall}
              className="cursor-pointer border-0 font-bold text-[15px] text-[#04140d] py-[14px] px-[26px] rounded-[14px] transition-all duration-200 hover:-translate-y-[3px]"
              style={{
          ...
          <div data-reveal className="flex gap-[28px] flex-wrap mt-[44px]">
            {hero.trustStrip.map((item, idx) => (
              <div key={item.id} className="flex gap-[28px]">
                {idx > 0 && <div className="w-[1px] bg-[rgba(255,255,255,0.1)]"></div>}
                <div>
                  <div className={`font-display font-bold text-[24px] ${idx === hero.trustStrip.length - 1 ? 'gradient-text' : 'text-[#f4f9f6]'}`}>
                    {item.n}
                  </div>
                  <div className="text-[12.5px] text-[#7f9089] tracking-[.03em]">{item.l}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex-[1_1_380px] min-w-[300px] flex justify-center relative">
          ...
          >
            ...
            <div className="p-[20px] pb-[24px] font-mono text-[13.5px] leading-[1.85]">
              ...
              <div className="pl-[20px]"><span className="text-[#f78c6c]">builds</span>: [{hero.codeSnippet.builds.map(b => `"${b}"`).join(', ')}],</div>
          ...
          <div
            data-float
            className="absolute top-[46%] -left-[14px] animate-float-a flex items-center gap-[8px] py-[9px] px-[14px] rounded-[12px] text-[12.5px] font-semibold text-[#e7efe9] shadow-[0_12px_30px_rgba(0,0,0,.4)]"
            style={{
              background: 'linear-gradient(155deg, rgba(255,255,255,0.1), rgba(255,255,255,0.03))',
              border: '1px solid rgba(255,255,255,0.14)',
              backdropFilter: 'blur(14px)',
